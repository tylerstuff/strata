...
fn pointVisibility(world: vec3f, dx: vec3f, dy: vec3f) -> f32 {
  ...
  for (var y = -1; y <= 1; y++) { for (var x = -1; x <= 1; x++) {
    ...
    sum += select(0.0, 1.0, reference <= textureLoad(importedPointDepth, pixel, adjacent.face, 0));
  } }
  return sum / 9.0;
}
fn pointDirect(world: vec3f, dx: vec3f, dy: vec3f, base: vec3f, roughness: f32, metallic: f32, normal: vec3f, view: vec3f) -> vec3f {
  ...
  return evaluateDirectLight(base, roughness, metallic, normal, view, delta / distance, incident) * pointVisibility(world, dx, dy);
}
`;
