...
The runtime currently implements initialization, a Rust/WASM worker, diffuse/PBR rendering, directional shadows, temporal anti-aliasing, static analytic-terrain streaming, separate two-room software-traced diffuse GI and selected-reflection experiments, CPU/GPU telemetry, resize, and disposal. A restricted integrated courtyard shares one camera/shadow/MRT/lighting path and retains a separate coarse terrain tracing proxy; general scene authoring and arbitrary imported virtual geometry remain planned. Read docs/benchmark.md for measurement and external-result rules, docs/raster.md for PBR, docs/gi.md and docs/reflections.md for the restricted lighting proofs, and docs/virtual-geometry.md plus docs/geometry-format.md for the restricted geometry proof. Read docs/integrated.md for the shared courtyard, explicit tracing approximation, fixed probe scope and mixed current/delayed triangle counts. Read docs/runtime.md for build, package, ownership, and validation details. Use `npm run check` for the complete checks; consumer tests require a browser with WebGPU and a built package. Do not describe planned graphics features or performance targets as implemented or achieved.
