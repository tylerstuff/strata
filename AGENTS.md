...
The runtime currently implements initialization, a Rust/WASM worker, diffuse/PBR rendering, directional shadows, temporal anti-aliasing, static analytic-terrain streaming, a separate two-room software-traced diffuse GI experiment, CPU/GPU telemetry, resize, and disposal. Integrated scene lighting, reflections and arbitrary imported virtual geometry remain planned. Read docs/benchmark.md for measurement and external-result rules, docs/raster.md for PBR, docs/gi.md for the restricted lighting proof, and docs/virtual-geometry.md plus docs/geometry-format.md for the restricted geometry proof. Read docs/runtime.md for build, package, ownership, and validation details. Use `npm run check` for the complete checks; consumer tests require a browser with WebGPU and a built package. Do not describe planned graphics features or performance targets as implemented or achieved.
