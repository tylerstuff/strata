...
The runtime currently implements initialization, a Rust/WASM worker, diffuse/PBR procedural rendering, directional shadows, temporal anti-aliasing, CPU/GPU telemetry, resize, and disposal. Advanced graphics remain planned. Read docs/benchmark.md for measurement and external-result rules, and docs/raster.md for the supported raster path. Read docs/runtime.md for build, package, ownership, and validation details. Use `npm run check` for the complete checks; consumer tests require a browser with WebGPU and a built package. Do not describe planned graphics features or performance targets as implemented or achieved.
