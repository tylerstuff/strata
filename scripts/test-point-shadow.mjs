...
const output = `${homedir()}/Downloads/Strata-Benchmark-Results/${new Date().toISOString().replaceAll(':','-')}-point-shadow`;
...
const before = source();
const bundle = await build({entryPoints:['tests/browser/point-shadow-validation.ts'], write:false,
...
try {
  ...
  await page.route(`${server.url}/point-shadow.js`, route => route.fulfill({contentType:'text/javascript',body:bundle.outputFiles[0].text}));
  await page.goto(`${server.url}/benchmarks/browser/index.html`);
  const report = await page.evaluate(async()=>{const mod=await import('/point-shadow.js');return mod.validatePointShadow();});
  ...
  console.log('Point shadow validation',report.status,output);
