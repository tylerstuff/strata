...
if (process.argv.length !== 2) throw new Error('Usage: node scripts/test-imported-indirect.mjs');
...
const harnessPath = 'tests/browser/imported-indirect-validation.ts';
const harness = await readFile(join(root, harnessPath));
const runner = await readFile(fileURLToPath(import.meta.url));
const fixtureNames = ['imported-static-bvh.json', 'imported-indirect-environment.json'];
...
const output = join(homedir(), 'Downloads/Strata-Benchmark-Results', `${new Date().toISOString().replaceAll(':', '-')}-imported-indirect-validation`);
...
const report = { kind: 'strata-imported-progressive-indirect-generated', status: 'running', performanceEvidence: false,
  ...
  limitations: ['Only generated geometry and immutable generated environment references are loaded. No external model collection is served or copied.',
    ...
    'Explicit-point batch uses singular synthetic matrices, separately labelled from the valid orthographic-camera effect regression.',
    'Same-ray numerical agreement is the acceptance oracle. Fixed shader hash samples are not described as independent Monte Carlo confidence evidence.'] };
...
try {
  ...
  await page.route(`${server.url}/imported-indirect.html`, route => route.fulfill({ contentType: 'text/html', body: '<!doctype html><title>Strata generated imported indirect checks</title>' }));
  await page.route(`${server.url}/imported-indirect.js`, route => route.fulfill({ contentType: 'text/javascript', body: bundle.outputFiles[0].text }));
  ...
  await page.goto(`${server.url}/imported-indirect.html`);
  report.result = await Promise.race([
    page.evaluate(async () => (await import('/imported-indirect.js')).validateImportedIndirect()),
    new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('Imported indirect validation exceeded120seconds.')), 120_000); }),
  ]);
  ...
  console.log(`Imported indirect generated checks ${report.status}. External report: ${output}/report.json`);
}
