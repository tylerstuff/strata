...
export async function finalizeBake(input,output) {
  ...
  for(const mesh of baked.meshes)for(const primitive of mesh.primitives){
    if(primitive.attributes.COLOR_0===undefined)throw Error('Export omitted baked vertex colors.');
  ...
  baked.extensionsUsed=[...new Set([...(source.extensionsUsed??[]),...(baked.extensionsUsed??[]),'KHR_materials_emissive_strength'])];
