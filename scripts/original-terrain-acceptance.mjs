...
export async function prepareOriginalTerrain(args){
  ...
  const result={kind:'original-terrain-acceptance-cpu-preparation-v1',runnable:false,source:{root:await realpath(repository),commit,status},
    ...
    remaining:['Test-owned native frame/cache/feedback observer has CPU coverage; browser integration and loaded-module admission remain absent','Lossless raw-attachment archive is an external draft; codec, ring, persistence and reference oracle integration remain absent','C top-down waypoint coverage witnesses remain separate proposals and are not wired','No frozen runtime build/server/browser launcher','No GPU grant or executed acceptance evidence'],defaultPolicy:'greedy',qualityAccepted:false};
