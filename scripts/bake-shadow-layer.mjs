...
if(r===''||(r!=='..'&&!r.startsWith('..'+sep)&&!isAbsolute(r)))throw Error('Shadow layer output must be external to the repository.');
try{await access(output);throw Error('Output already exists.');}catch(e){if(e.code!=='ENOENT')throw e;}
await mkdir(output);
const child=spawn(values.blender,['--background','--factory-startup','--python-exit-code','1','--python',resolve(repository,'scripts/bake-shadow-layer.py'),'--',input,config,output],{stdio:'inherit'});
const code=await new Promise((resolve,reject)=>{child.on('error',reject);child.on('exit',resolve);});
if(code!==0)throw Error('Shadow layer bake failed; partial output is not a completed bake.');
console.log('Completed shadow layer volume: '+output+'/scene.gltf');
