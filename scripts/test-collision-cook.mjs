...
import {createCapsuleController,decodeCollisionSnapshot} from '../packages/core/dist/gameplay.js';
...
try{
 ...
 execFileSync(process.execPath,['scripts/cook-collision.mjs','--input',join(root,'scene.gltf'),'--output',join(root,'compressed'),'--gzip'],{stdio:'pipe'});
 const compressed=JSON.parse(await readFile(join(root,'compressed/collision.json'),'utf8'));const restored=await decodeCollisionSnapshot(new Uint8Array(await readFile(join(root,'compressed/world.bin.gz'))),compressed.transport);
 assert.deepEqual(Buffer.from(restored),await readFile(join(root,'cooked/world.bin')));
 await assert.rejects(readFile(join(root,'compressed/world.bin')),e=>e.code==='ENOENT');
