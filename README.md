...
Application code can now [create fixed-topology mesh assets and submit batched root transforms](docs/meshes.md) through the shared PBR/shadow/motion path. This is an initial conventional mesh slice; transparent materials, dynamic geometry and general scene authoring remain pending.
...
An optional [gameplay foundation](docs/gameplay.md) adds fixed-step character movement, authored box colliders and an optional internally initialized WASM capsule/triangle backend with slope and step handling. A generated playable course and offline collision cooker exercise the public APIs. Static sphere sweeps support follow-camera obstruction using the same collision world. Collision simplification/streaming, skinned animation blending and broader camera behavior remain pending under #71.
