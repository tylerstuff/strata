...
Smoke mode allows a bounded 30-second final timestamp flush for software adapters; performance mode retains the five-second deadline. The flush occurs after measurement. Capture settling also fences submitted GPU work, with a 120-second deadline, so an unavailable profiler does not permit screenshots of incomplete work. Smoke screenshots allow 120 seconds for browser composition; performance screenshots retain 15 seconds. Capture failures preserve completed measurement samples and bounded diagnostics but invalidate the run. The small-resolution GI validation harness separately observes 240 submissions and reports stabilization relative to the final 24-frame tail; this is not radiometric convergence against an independent lighting reference. The legacy report key `convergence` is retained for compatibility and includes an explicit `qualification` describing this limit. Full-resolution smoke screenshots check package/render/report operation only.
...
GPU profiling uses a bounded sixteen-frame readback ring (5120 bytes of query-resolve/readback buffers). It never waits in the normal submission path. `gpuProfiling` in telemetry separates frame-level `skippedFrames` (ring pressure), `failedReadbacks`, `evictedFrames` (undrained result queue), and `cancelledFrames`; `pendingFrames` and `ringCapacity` describe occupancy. Existing pending/dropped counters remain pass-sample counts, so they are not interchangeable with these frame counts. The result queue stays bounded to 256 pass samples. A deeper ring improves tolerance for delayed browser map callbacks, but neither guarantees complete coverage nor fixes GPU execution cost. Record coverage against the actual measured frame IDs.
