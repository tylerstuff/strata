...
Existing combined Bistro floor lightmaps remain fixed: characters can receive the new lamp shadows and cast onto unbaked local receivers, but cannot darken an individual baked lamp's contribution on that floor. That requires separate per-light shadow masks or a decomposed direct-light bake. The actor demonstration uses a point approximation for one nearby lantern and indirect-only probes from all emissive surfaces; it does not reproduce direct illumination from every street lamp.
