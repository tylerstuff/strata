...
`npm run test:probe-volume` exercises the production sampling shader against synthetic atlases, explicit world-space boundary cases, and a negative control with only the volume guard removed. `npm run test:gi` includes this check after the existing tracing/probe/renderer checks, including in CI. The synthetic check establishes the sampling support contract; it does not validate physical outdoor illumination or solve in-volume visibility error.
