...
The optional authoring package supplies versioned scene documents, typed edits and revision-checked file operations. The separate preview package lowers supported opaque procedural root boxes through Core's public validator and lifecycle, then captures revision-bound browser evidence. Its browser bridge imports Core alone. Imported assets and persistent project connections are outside this first preview profile; they do not block the bounded authored-box workflow.
