...
The optional authoring package supplies versioned scene documents, typed edits and revision-checked file operations. The separate preview package lowers supported opaque procedural root boxes through Core's public validator and lifecycle, then captures revision-bound browser evidence. Its browser bridge imports Core alone. Imported assets remain outside this authored preview profile.

The optional [local preview connection](preview-connection.md) exposes that existing session through a process-owned stdio protocol and the `@strata-engine/preview/connection` Node export. It scopes scene reads and capture output to fixed roots, creates its browser lazily, and retains operation ownership through cancellation and bounded cleanup. It introduces no runtime scene model, network control service, scene-editing API or additional renderer capability. Authoring edits continue through the shared authoring package. Its CPU and restricted installed-browser checks passed; the guide records exact checkpoints, environments and limits.
