...
The shader reads precomputed face aprons through a 2D-array view with hardware
bilinear filtering and explicit mip interpolation. Aprons crossing one face edge contain the corresponding
...
Explicit adjacent-face aprons avoid this cube-sampler dependency while retaining the canonical radiance/roughness data. The only added source quantization is binary16 rounding of corner means; 2D-array filtering can also round interpolation weights. This evidence identifies
...
The canonical generated payload remains 540640 bytes: two cubes at 262128 bytes each and a
16384-byte DFG lookup. At load time, the seven levels receive one-texel adjacent-face aprons and are packed vertically into a 66×141×12 RGBA16F array. GPU texture allocation is 909760 bytes, including the DFG; actual level-rectangle uploads total 592096 bytes. Hardware bilinear sampling uses those aprons and explicit interpolation between roughness levels. Cube corners store the mean of their three incident face texels, rounded once to binary16; numeric filtering acceptance allows 0.001 linear error on the generated seam/corner witnesses. Source texels remain bit-identical. A 176-byte uniform holds the selected SH, mode, intensity
...
texels (eight bytes) and all 909760 environment bytes, even when unlit/off.
...
mutations that reverse face-row orientation, discard adjacent-face color, or discard mip
