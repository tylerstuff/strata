...
Every object/world change currently refits the BVH and resets the entire diffuse probe epoch. Continuous movement can therefore repeatedly prevent lighting stabilization. Specular camera motion resets reflection history because surface motion alone does not model reflected parallax. The measured tour exposes these as quality failures. Continuous object changes restart the probe frontier, starving most probes, and reset whole-image TAA. They must be fixed and measured using moving images, not hidden by choosing only still frames. Follow-up issues [#15](https://github.com/tylerstuff/strata/issues/15) and [#17](https://github.com/tylerstuff/strata/issues/17) track diffuse and reflection quality.
