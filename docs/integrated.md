...
This experimental renderer combines streamed terrain, conventional room geometry, world-space diffuse GI and selected software reflections in one camera image. It is a restricted integration test, not a general scene graph or an established 60 FPS game-quality result. The [first M2 feasibility report](benchmarks/2026-09-05-m2-integrated.md) records the complete primary feature comparisons, visible failures and follow-up work. The overall target is not achieved.
...
The shared trace source uses [incremental trace updates](incremental-trace-updates.md). A moving box regenerates its 12 triangles and the affected BVH bounds while retaining the static terrain proxy. Pending writes coalesce whole adjacent records, with a bounded full-buffer fallback for fragmented plans. GPU allocation sizes are unchanged; the added CPU maintenance payload is reported separately as `gi.traceMetadataBytes`. Record and upload reductions do not establish a frame-time improvement.
...
In the first recorded tour, every object/world change refit the complete BVH and reset the entire diffuse probe epoch. Continuous movement repeatedly prevented lighting stabilization. Current object-only motion retains bounded diffuse rolling refresh, while hard world changes still invalidate that cache; see [gi-motion.md](gi-motion.md). Trace maintenance now preserves static triangle records during narrow updates. Specular camera motion still resets reflection history because surface motion alone does not model reflected parallax. Object motion also resets reflection and whole-image TAA history. These changes need matched moving-image evaluation; they do not establish that the original tour's quality failures are resolved. Issues [#15](https://github.com/tylerstuff/strata/issues/15) and [#17](https://github.com/tylerstuff/strata/issues/17) track diffuse and reflection quality.
