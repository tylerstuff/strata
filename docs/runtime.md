...
The profiler has four asynchronous slots, each supporting up to ten named passes (1,280 bytes of requested resolve/readback buffer payload in total). The default GI + world-reflection + TAA path records nine passes; the integrated GPU geometry path adds selection for ten. Unused capacity does not create additional passes or waits.
