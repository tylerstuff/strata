...
The GI, reflection and integrated fixtures maintain changed trace records in TypeScript without a worker boundary. [Incremental trace updates](incremental-trace-updates.md) documents the fixed-topology contract, CPU metadata, partial-write recovery and flat `gi.trace*` counters. Those counters distinguish prepared CPU targets, completed queue-write plans and submitted source versions; none is a GPU-completion measurement.
