...
The ordinary six-binding path's composed `rgba16float` presentation texture
saturates each finite RGB channel at 65,504. This storage limit applies after
direct plus indirect addition; the float32 raw estimator, sample counts and failure
counters are unchanged. Lower
...
and this step does not promise to repair earlier nonfinite values. The optional
shared-primary capability below uses its separate conservative domain and explicit
presentation fault instead of this saturation policy.
...
record word. This capability leaves the current ordinary six-binding
`legacy-inline-primary-v1` transport and its 65,504 presentation saturation intact.
...
This is an experimental option whose actual-house visual quality remains
unaccepted. That quality assessment is separate from integration of independently
validated engine changes. It has no real-time performance, general denoising
